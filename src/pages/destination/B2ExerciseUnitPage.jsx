...
function getIframeSrc(key) {
  if (/^review\d+$/.test(key)) return `/b2-units/${key}/index.html`
  if (/^progress-test-\d+$/.test(key)) return `/b2-units/${key}/index.html`
  const m = key.match(/\d+/)
  if (m) return `/b2-units/unit${m[0]}/index.html`
  return null
}
...
function formatLabel(key) {
  if (/^review(\d+)$/.test(key)) return `Review ${key.match(/\d+/)[0]} – Destination B2`
  if (/^progress-test-(\d+)$/.test(key)) return `Progress Test ${key.match(/\d+/)[0]} – Destination B2`
  return `${key} – Destination B2`
}

export default function B2ExerciseUnitPage() {
  const { unitKey } = useParams()
  const navigate = useNavigate()
  const key = decodeURIComponent(unitKey)
  const src = getIframeSrc(key)
  console.log("duong dan: " + src)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <div style={{
        ...
        <button
          onClick={() => navigate('/destination/b2/exercises')}
          style={{
