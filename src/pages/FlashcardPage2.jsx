...
export default function FlashcardPage2() {
  ...
  const { destination, unitId, source, reviewMode, words: stateWords, studySetName } = location.state || {}

  // 1. CHUYỂN CARDS THÀNH STATE VÀ THÊM TRẠNG THÁI LOADING
  ...
  // 2. DÙNG USEEFFECT ĐỂ ĐỢI GỌI API ẢNH ASYNC
  useEffect(() => {
    async function loadCards() {
      ...
      try {
        ...
        if (reviewMode) {
          ...
          // Thêm await vì buildFlashcards giờ chứa logic async lấy ảnh
          generatedCards = await buildFlashcards(dueVocab)
        } else if (stateWords) {
          if (stateWords[0]?.definition) {
            generatedCards = await buildFlashcardsFromFolder(stateWords)
          } else {
            generatedCards = await buildFlashcards(stateWords)
          }
        }
        setCards(generatedCards)

      } catch (error) {
        console.error("Lỗi khi build flashcards:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  // 3. HIỂN THỊ MÀN HÌNH LOADING TRONG LÚC CHỜ API TẢI ẢNH
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mb-4"></div>
        <p className="text-gray-500 text-sm">Đang chuẩn bị thẻ học và ảnh...</p>
      </div>
    )
  }
  ...
  const goBackToUnit = () => {
    ...
    if (unitId) {
      // Nếu có unitId -> quay về unit đó (ví dụ: /category/unit3)
      navigate(`/destination/${destination}/vocab/${unitId}`)
    } else {
      // Fallback: nếu không có unitId thì về trang chủ
      navigate('/')
    }
  }
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-xl mx-auto px-4 py-8 flex flex-col items-center gap-6">
          ...
          <div className="flex gap-3 w-full">
            <button
              onClick={() => { setIndex(0); setKnown(0); setUnknown(0); setDone(false); setReviewedWords(new Set())}}
              className="flex-1 py-3 bg-gray-200 text-gray-700 rounded-xl font-medium"
            >
              Thử lại
            </button>
            <button
              onClick={goBackToUnit}
              className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-medium"
            >
              {unitId ? 'Quay lại Unit' : 'Trang chủ'}
            </button>
          </div>
        </div>
      </div>
  ...
  const handleSwipeRight = () => {
    // Chỉ ghi SRS lần đầu
    if (!reviewedWords.has(currentCard.word)) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-3 mt-6">
          {ToastComponent}
          <button
            onClick={() => { if (index === 0) showToast('Đang ở card đầu tiên!', 'error'); else setIndex(i => i - 1) }}
